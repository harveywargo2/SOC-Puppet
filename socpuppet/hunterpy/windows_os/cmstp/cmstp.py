...
def cmstp_p1000_executing_inf_file_silent(*, type='m365d', kql_ago='1d'):

    if type == 'm365d':
        with open(os.path.join(cmstp_kql_path,
                               'cmstp_p1000_executing_inf_file_silent.yaml'), 'r') as file:
...
def cmstp_p1001_executing_inf_file(*, type='m365d', kql_ago='1d'):

    if type == 'm365d':
        with open(os.path.join(cmstp_kql_path,
                               'cmstp_p1001_executing_inf_file.yaml'), 'r') as file:
            data = yaml.safe_load(file)
...
def cmstp_p1002_executing_inf_file_all_profiles(*, type='m365d', kql_ago='1d'):
    ...
    if type == 'm365d':
        with open(os.path.join(cmstp_kql_path,
                               'cmstp_p1002_executing_inf_all_profiles.yaml'), 'r') as file:
            data = yaml.safe_load(file)
